...
安信通是只依赖内部局域网的政企即时通信平台，以组织、部门、岗位和人员目录为业务基座，再建立账号、设备、在线状态、单聊、群聊、文件与审计。系统需要在 100～50000 用户范围内按部署等级扩展，支持 Windows 与国产 Linux 客户端、PostgreSQL 服务端基线、国密通信能力和离线升级。
