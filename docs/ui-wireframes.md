...
```text
┌──────────────────────────────────────────────────────────────────────────────────────────────┐
│ OrgLink 安域通   当前组织：某某单位                         在线   张三   设置   最小化   关闭 │
├──────┬──────────────────────────────┬────────────────────────────────────────────────────────┤
...
- 公共图标由 `view/common/UiAssets` 在 24×24 标准坐标中使用 `QPainter` 绘制，不再用 Unicode 字符模拟图形，因此不依赖目标机器的图标字体。
- 尺寸与参考稿统一：主导航 24 px、常规工具栏 20 px、输入框前导/尾随图标 18 px、状态与紧凑列表 16 px、功能入口 32 px；高 DPI 下按窗口设备像素比生成像素图。
