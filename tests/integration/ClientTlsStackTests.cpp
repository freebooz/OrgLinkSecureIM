...
private slots:
    void twoClientsOverTls()
    {
        ...
        if (qEnvironmentVariableIntValue("ORGLINK_TEST_EXPECT_SETTINGS") == 1)
        {
            ...
            QCOMPARE(adminSystemInfo->storageDocumentBytes + adminSystemInfo->storageImageBytes
                + adminSystemInfo->storageVideoBytes + adminSystemInfo->storageOtherBytes,
                adminSystemInfo->storageUsedBytes);
            ...
            // 隐私范围、手机号检索开关与个性签名必须和通用设置共享修订号及审计事务。
            candidate.phoneVisibility = 1;
            candidate.emailVisibility = 2;
            candidate.searchVisibility = 1;
            candidate.phoneSearchEnabled = false;
            candidate.profileSignature = QStringLiteral("TLS 账号资料回归签名");
            // 消息与通知设置必须在同一乐观修订事务内保存，避免多端分项更新彼此覆盖。
            candidate.newMessageNotificationEnabled = false;
            candidate.notificationSoundEnabled = false;
            candidate.notificationSoundName = QStringLiteral("soft");
            candidate.desktopPopupEnabled = false;
            candidate.unreadBadgeEnabled = false;
            candidate.mentionNotificationEnabled = false;
            candidate.groupNotificationLevel = 2;
            candidate.systemNotificationEnabled = false;
            candidate.approvalNotificationEnabled = false;
            candidate.fileNotificationEnabled = false;
            candidate.calendarNotificationEnabled = false;
            candidate.calendarReminderMinutes = 30;
            candidate.doNotDisturbEnabled = true;
            candidate.doNotDisturbStartMinutes = 1'320;
            candidate.doNotDisturbEndMinutes = 480;
            candidate.notificationPreviewMode = 2;
            candidate.readReceiptEnabled = false;
            candidate.enterToSendEnabled = true;
            candidate.messageBubbleDensity = 2;
            candidate.primaryColor = QStringLiteral("#4E8CFF");
            candidate.accentColor = QStringLiteral("#13C2C2");
            candidate.sidebarStyle = 3;
            candidate.cardRadiusMode = 2;
            candidate.uiDensity = 0;
            candidate.fontSizeMode = 3;
            candidate.chatBackground = QStringLiteral("aurora");
            candidate.messageBubbleStyle = 1;
            candidate.contentViewMode = 1;
            candidate.windowTransparency = 35;
            candidate.animationEnabled = false;
            candidate.animationIntensity = 2;
            // 文件与存储偏好同样复用完整快照和 revision，分类用量则保持服务端只读聚合。
            candidate.autoSaveReceivedFiles = false;
            candidate.recentFileRetentionDays = 90;
            candidate.autoCacheCleanupEnabled = false;
            candidate.cacheSizeLimitMb = 5'120;
            candidate.filePreviewMode = 1;
            candidate.imageAutoCompressEnabled = false;
            candidate.videoTranscodeMode = 1;
            candidate.fileEncryptionMode = 1;
            candidate.externalWatermarkMode = 1;
            candidate.defaultSharePermission = 2;
            candidate.syncFolderPath = QStringLiteral("D:/OrgLink/Sync");
            candidate.echoCancellationEnabled = false;
            candidate.noiseSuppressionEnabled = false;
            candidate.autoGainControlEnabled = false;
            candidate.cameraMirrorEnabled = true;
            candidate.videoResolutionMode = 2;
            candidate.bandwidthOptimizationEnabled = false;
            candidate.recordingPermissionEnabled = true;
            candidate.incomingCallWindowPosition = 3;
            candidate.bluetoothPreferred = false;
            candidate.callShortcut = QStringLiteral("Ctrl+Alt+C");
            ...
            QCOMPARE(updatedSettings->phoneVisibility, candidate.phoneVisibility);
            QCOMPARE(updatedSettings->emailVisibility, candidate.emailVisibility);
            QCOMPARE(updatedSettings->searchVisibility, candidate.searchVisibility);
            QCOMPARE(updatedSettings->phoneSearchEnabled, candidate.phoneSearchEnabled);
            QCOMPARE(updatedSettings->profileSignature, candidate.profileSignature);
            QCOMPARE(updatedSettings->newMessageNotificationEnabled, candidate.newMessageNotificationEnabled);
            QCOMPARE(updatedSettings->notificationSoundEnabled, candidate.notificationSoundEnabled);
            QCOMPARE(updatedSettings->notificationSoundName, candidate.notificationSoundName);
            QCOMPARE(updatedSettings->desktopPopupEnabled, candidate.desktopPopupEnabled);
            QCOMPARE(updatedSettings->unreadBadgeEnabled, candidate.unreadBadgeEnabled);
            QCOMPARE(updatedSettings->mentionNotificationEnabled, candidate.mentionNotificationEnabled);
            QCOMPARE(updatedSettings->groupNotificationLevel, candidate.groupNotificationLevel);
            QCOMPARE(updatedSettings->systemNotificationEnabled, candidate.systemNotificationEnabled);
            QCOMPARE(updatedSettings->approvalNotificationEnabled, candidate.approvalNotificationEnabled);
            QCOMPARE(updatedSettings->fileNotificationEnabled, candidate.fileNotificationEnabled);
            QCOMPARE(updatedSettings->calendarNotificationEnabled, candidate.calendarNotificationEnabled);
            QCOMPARE(updatedSettings->calendarReminderMinutes, candidate.calendarReminderMinutes);
            QCOMPARE(updatedSettings->doNotDisturbEnabled, candidate.doNotDisturbEnabled);
            QCOMPARE(updatedSettings->doNotDisturbStartMinutes, candidate.doNotDisturbStartMinutes);
            QCOMPARE(updatedSettings->doNotDisturbEndMinutes, candidate.doNotDisturbEndMinutes);
            QCOMPARE(updatedSettings->notificationPreviewMode, candidate.notificationPreviewMode);
            QCOMPARE(updatedSettings->readReceiptEnabled, candidate.readReceiptEnabled);
            QCOMPARE(updatedSettings->enterToSendEnabled, candidate.enterToSendEnabled);
            QCOMPARE(updatedSettings->messageBubbleDensity, candidate.messageBubbleDensity);
            QCOMPARE(updatedSettings->primaryColor, candidate.primaryColor);
            QCOMPARE(updatedSettings->accentColor, candidate.accentColor);
            QCOMPARE(updatedSettings->sidebarStyle, candidate.sidebarStyle);
            QCOMPARE(updatedSettings->cardRadiusMode, candidate.cardRadiusMode);
            QCOMPARE(updatedSettings->uiDensity, candidate.uiDensity);
            QCOMPARE(updatedSettings->fontSizeMode, candidate.fontSizeMode);
            QCOMPARE(updatedSettings->chatBackground, candidate.chatBackground);
            QCOMPARE(updatedSettings->messageBubbleStyle, candidate.messageBubbleStyle);
            QCOMPARE(updatedSettings->contentViewMode, candidate.contentViewMode);
            QCOMPARE(updatedSettings->windowTransparency, candidate.windowTransparency);
            QCOMPARE(updatedSettings->animationEnabled, candidate.animationEnabled);
            QCOMPARE(updatedSettings->animationIntensity, candidate.animationIntensity);
            QCOMPARE(updatedSettings->autoSaveReceivedFiles, candidate.autoSaveReceivedFiles);
            QCOMPARE(updatedSettings->recentFileRetentionDays, candidate.recentFileRetentionDays);
            QCOMPARE(updatedSettings->autoCacheCleanupEnabled, candidate.autoCacheCleanupEnabled);
            QCOMPARE(updatedSettings->cacheSizeLimitMb, candidate.cacheSizeLimitMb);
            QCOMPARE(updatedSettings->filePreviewMode, candidate.filePreviewMode);
            QCOMPARE(updatedSettings->imageAutoCompressEnabled, candidate.imageAutoCompressEnabled);
            QCOMPARE(updatedSettings->videoTranscodeMode, candidate.videoTranscodeMode);
            QCOMPARE(updatedSettings->fileEncryptionMode, candidate.fileEncryptionMode);
            QCOMPARE(updatedSettings->externalWatermarkMode, candidate.externalWatermarkMode);
            QCOMPARE(updatedSettings->defaultSharePermission, candidate.defaultSharePermission);
            QCOMPARE(updatedSettings->syncFolderPath, candidate.syncFolderPath);
            QCOMPARE(updatedSettings->echoCancellationEnabled, candidate.echoCancellationEnabled);
            QCOMPARE(updatedSettings->noiseSuppressionEnabled, candidate.noiseSuppressionEnabled);
            QCOMPARE(updatedSettings->autoGainControlEnabled, candidate.autoGainControlEnabled);
            QCOMPARE(updatedSettings->cameraMirrorEnabled, candidate.cameraMirrorEnabled);
            QCOMPARE(updatedSettings->videoResolutionMode, candidate.videoResolutionMode);
            QCOMPARE(updatedSettings->bandwidthOptimizationEnabled, candidate.bandwidthOptimizationEnabled);
            QCOMPARE(updatedSettings->recordingPermissionEnabled, candidate.recordingPermissionEnabled);
            QCOMPARE(updatedSettings->incomingCallWindowPosition, candidate.incomingCallWindowPosition);
            QCOMPARE(updatedSettings->bluetoothPreferred, candidate.bluetoothPreferred);
            QCOMPARE(updatedSettings->callShortcut, candidate.callShortcut);
            // 隔离性验证只比较本轮写入前后的修订，不假设共享开发库中的用户从未被其他回归更新。
            QCOMPARE(userSettings->revision, userPreviousRevision);
            ...
            QCOMPARE(resetSettings->phoneVisibility, 0);
            QCOMPARE(resetSettings->emailVisibility, 0);
            QCOMPARE(resetSettings->searchVisibility, 0);
            QVERIFY(resetSettings->phoneSearchEnabled);
            QVERIFY(resetSettings->profileSignature.isEmpty());
            QVERIFY(resetSettings->newMessageNotificationEnabled);
            QVERIFY(resetSettings->notificationSoundEnabled);
            QCOMPARE(resetSettings->notificationSoundName, QStringLiteral("default"));
            QVERIFY(resetSettings->desktopPopupEnabled);
            QVERIFY(resetSettings->unreadBadgeEnabled);
            QVERIFY(resetSettings->mentionNotificationEnabled);
            QCOMPARE(resetSettings->groupNotificationLevel, 0);
            QVERIFY(resetSettings->systemNotificationEnabled);
            QVERIFY(resetSettings->approvalNotificationEnabled);
            QVERIFY(resetSettings->fileNotificationEnabled);
            QVERIFY(resetSettings->calendarNotificationEnabled);
            QCOMPARE(resetSettings->calendarReminderMinutes, 15);
            QVERIFY(!resetSettings->doNotDisturbEnabled);
            QCOMPARE(resetSettings->doNotDisturbStartMinutes, 1'320);
            QCOMPARE(resetSettings->doNotDisturbEndMinutes, 480);
            QCOMPARE(resetSettings->notificationPreviewMode, 0);
            QVERIFY(resetSettings->readReceiptEnabled);
            QVERIFY(!resetSettings->enterToSendEnabled);
            QCOMPARE(resetSettings->messageBubbleDensity, 1);
            QCOMPARE(resetSettings->primaryColor, QStringLiteral("#1677FF"));
            QCOMPARE(resetSettings->accentColor, QStringLiteral("#13C2C2"));
            QCOMPARE(resetSettings->sidebarStyle, 0);
            QCOMPARE(resetSettings->cardRadiusMode, 1);
            QCOMPARE(resetSettings->uiDensity, 1);
            QCOMPARE(resetSettings->fontSizeMode, 1);
            QCOMPARE(resetSettings->chatBackground, QStringLiteral("default"));
            QCOMPARE(resetSettings->messageBubbleStyle, 0);
            QCOMPARE(resetSettings->contentViewMode, 0);
            QCOMPARE(resetSettings->windowTransparency, 30);
            QVERIFY(resetSettings->animationEnabled);
            QCOMPARE(resetSettings->animationIntensity, 1);
            QVERIFY(resetSettings->autoSaveReceivedFiles);
            QCOMPARE(resetSettings->recentFileRetentionDays, 30);
            QVERIFY(resetSettings->autoCacheCleanupEnabled);
            QCOMPARE(resetSettings->cacheSizeLimitMb, 2'048);
            QCOMPARE(resetSettings->filePreviewMode, 0);
            QVERIFY(resetSettings->imageAutoCompressEnabled);
            QCOMPARE(resetSettings->videoTranscodeMode, 0);
            QCOMPARE(resetSettings->fileEncryptionMode, 0);
            QCOMPARE(resetSettings->externalWatermarkMode, 0);
            QCOMPARE(resetSettings->defaultSharePermission, 0);
            QVERIFY(resetSettings->syncFolderPath.isEmpty());
            QVERIFY(resetSettings->echoCancellationEnabled);
            QVERIFY(resetSettings->noiseSuppressionEnabled);
            QVERIFY(resetSettings->autoGainControlEnabled);
            QVERIFY(!resetSettings->cameraMirrorEnabled);
            QCOMPARE(resetSettings->videoResolutionMode, 1);
            QVERIFY(resetSettings->bandwidthOptimizationEnabled);
            QVERIFY(!resetSettings->recordingPermissionEnabled);
            QCOMPARE(resetSettings->incomingCallWindowPosition, 0);
            QVERIFY(resetSettings->bluetoothPreferred);
            QCOMPARE(resetSettings->callShortcut, QStringLiteral("Alt+C"));
        }
    }
};
