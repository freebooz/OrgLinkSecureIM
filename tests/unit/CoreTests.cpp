...
void testProtocolFraming()
{
    ...
    // 解析调用不能放在 assert 表达式内，否则 Release 的 NDEBUG 会跳过半包输入并让后续字节从中间开始解析。
    const auto partial = decoder.append(std::span(encoded).first(5));
...
void testApplicationMessageCodec()
{
    ...
    settingsResponse.settings.phoneVisibility = 1;
    settingsResponse.settings.emailVisibility = 2;
    settingsResponse.settings.searchVisibility = 0;
    settingsResponse.settings.phoneSearchEnabled = false;
    settingsResponse.settings.profileSignature = "专注安全通信";
    settingsResponse.settings.newMessageNotificationEnabled = true;
    settingsResponse.settings.notificationSoundEnabled = false;
    settingsResponse.settings.notificationSoundName = "soft";
    settingsResponse.settings.desktopPopupEnabled = true;
    settingsResponse.settings.unreadBadgeEnabled = false;
    settingsResponse.settings.mentionNotificationEnabled = true;
    settingsResponse.settings.groupNotificationLevel = 2;
    settingsResponse.settings.systemNotificationEnabled = false;
    settingsResponse.settings.approvalNotificationEnabled = true;
    settingsResponse.settings.fileNotificationEnabled = false;
    settingsResponse.settings.calendarNotificationEnabled = true;
    settingsResponse.settings.calendarReminderMinutes = 30;
    settingsResponse.settings.doNotDisturbEnabled = true;
    settingsResponse.settings.doNotDisturbStartMinutes = 1'320;
    settingsResponse.settings.doNotDisturbEndMinutes = 480;
    settingsResponse.settings.notificationPreviewMode = 2;
    settingsResponse.settings.readReceiptEnabled = false;
    settingsResponse.settings.enterToSendEnabled = true;
    settingsResponse.settings.messageBubbleDensity = 2;
    settingsResponse.settings.primaryColor = "#4E8CFF";
    settingsResponse.settings.accentColor = "#13C2C2";
    settingsResponse.settings.sidebarStyle = 3;
    settingsResponse.settings.cardRadiusMode = 2;
    settingsResponse.settings.uiDensity = 0;
    settingsResponse.settings.fontSizeMode = 3;
    settingsResponse.settings.chatBackground = "aurora";
    settingsResponse.settings.messageBubbleStyle = 1;
    settingsResponse.settings.contentViewMode = 1;
    settingsResponse.settings.windowTransparency = 35;
    settingsResponse.settings.animationEnabled = true;
    settingsResponse.settings.animationIntensity = 2;
    settingsResponse.settings.autoSaveReceivedFiles = true;
    settingsResponse.settings.recentFileRetentionDays = 90;
    settingsResponse.settings.autoCacheCleanupEnabled = false;
    settingsResponse.settings.cacheSizeLimitMb = 5'120;
    settingsResponse.settings.filePreviewMode = 1;
    settingsResponse.settings.imageAutoCompressEnabled = false;
    settingsResponse.settings.videoTranscodeMode = 1;
    settingsResponse.settings.fileEncryptionMode = 1;
    settingsResponse.settings.externalWatermarkMode = 1;
    settingsResponse.settings.defaultSharePermission = 2;
    settingsResponse.settings.syncFolderPath = "D:/OrgLink/Sync";
    settingsResponse.settings.echoCancellationEnabled = true;
    settingsResponse.settings.noiseSuppressionEnabled = false;
    settingsResponse.settings.autoGainControlEnabled = true;
    settingsResponse.settings.cameraMirrorEnabled = true;
    settingsResponse.settings.videoResolutionMode = 2;
    settingsResponse.settings.bandwidthOptimizationEnabled = false;
    settingsResponse.settings.recordingPermissionEnabled = true;
    settingsResponse.settings.incomingCallWindowPosition = 3;
    settingsResponse.settings.bluetoothPreferred = false;
    settingsResponse.settings.callShortcut = "Ctrl+Alt+C";
    ...
    settingsResponse.systemInfo.organizationName = "测试组织";
    settingsResponse.systemInfo.loginName = "alice";
    settingsResponse.systemInfo.accountStatusText = "正常";
    settingsResponse.systemInfo.lastLoginAtUtcMs = 1'725'000'000'000ULL;
    settingsResponse.systemInfo.lastLoginDeviceName = "Office PC";
    settingsResponse.systemInfo.lastLoginPlatform = "Windows";
    settingsResponse.systemInfo.lastLoginSource = "10.0.0.8";
    settingsResponse.systemInfo.teamMemberCount = 32;
    settingsResponse.systemInfo.storageDocumentBytes = 1024;
    settingsResponse.systemInfo.storageImageBytes = 512;
    settingsResponse.systemInfo.storageVideoBytes = 2048;
    settingsResponse.systemInfo.storageOtherBytes = 512;
    settingsResponse.systemInfo.syncedFileCount = 18;
    settingsResponse.systemInfo.lastFileSyncAtUtcMs = 1'725'000'100'000ULL;
    ...
    assert(decodedSettings.settings.emailVisibility == 2);
    assert(!decodedSettings.settings.phoneSearchEnabled);
    assert(decodedSettings.settings.profileSignature == "专注安全通信");
    // 通知偏好的关闭值、枚举和跨日免打扰分钟数都必须经过同一 wire 快照可逆传输。
    assert(decodedSettings.settings.newMessageNotificationEnabled);
    assert(!decodedSettings.settings.notificationSoundEnabled);
    assert(decodedSettings.settings.notificationSoundName == "soft");
    assert(!decodedSettings.settings.unreadBadgeEnabled);
    assert(decodedSettings.settings.groupNotificationLevel == 2);
    assert(!decodedSettings.settings.systemNotificationEnabled);
    assert(decodedSettings.settings.calendarReminderMinutes == 30);
    assert(decodedSettings.settings.doNotDisturbEnabled);
    assert(decodedSettings.settings.doNotDisturbStartMinutes == 1'320);
    assert(decodedSettings.settings.doNotDisturbEndMinutes == 480);
    assert(decodedSettings.settings.notificationPreviewMode == 2);
    assert(!decodedSettings.settings.readReceiptEnabled);
    assert(decodedSettings.settings.enterToSendEnabled);
    assert(decodedSettings.settings.messageBubbleDensity == 2);
    assert(decodedSettings.settings.primaryColor == "#4E8CFF");
    assert(decodedSettings.settings.accentColor == "#13C2C2");
    assert(decodedSettings.settings.sidebarStyle == 3);
    assert(decodedSettings.settings.cardRadiusMode == 2);
    assert(decodedSettings.settings.uiDensity == 0);
    assert(decodedSettings.settings.fontSizeMode == 3);
    assert(decodedSettings.settings.chatBackground == "aurora");
    assert(decodedSettings.settings.messageBubbleStyle == 1);
    assert(decodedSettings.settings.contentViewMode == 1);
    assert(decodedSettings.settings.windowTransparency == 35);
    assert(decodedSettings.settings.animationEnabled);
    assert(decodedSettings.settings.animationIntensity == 2);
    assert(decodedSettings.settings.autoSaveReceivedFiles);
    assert(decodedSettings.settings.recentFileRetentionDays == 90);
    assert(!decodedSettings.settings.autoCacheCleanupEnabled);
    assert(decodedSettings.settings.cacheSizeLimitMb == 5'120);
    assert(decodedSettings.settings.filePreviewMode == 1);
    assert(!decodedSettings.settings.imageAutoCompressEnabled);
    assert(decodedSettings.settings.videoTranscodeMode == 1);
    assert(decodedSettings.settings.fileEncryptionMode == 1);
    assert(decodedSettings.settings.externalWatermarkMode == 1);
    assert(decodedSettings.settings.defaultSharePermission == 2);
    assert(decodedSettings.settings.syncFolderPath == "D:/OrgLink/Sync");
    assert(decodedSettings.settings.echoCancellationEnabled);
    assert(!decodedSettings.settings.noiseSuppressionEnabled);
    assert(decodedSettings.settings.autoGainControlEnabled);
    assert(decodedSettings.settings.cameraMirrorEnabled);
    assert(decodedSettings.settings.videoResolutionMode == 2);
    assert(!decodedSettings.settings.bandwidthOptimizationEnabled);
    assert(decodedSettings.settings.recordingPermissionEnabled);
    assert(decodedSettings.settings.incomingCallWindowPosition == 3);
    assert(!decodedSettings.settings.bluetoothPreferred);
    assert(decodedSettings.settings.callShortcut == "Ctrl+Alt+C");
    assert(decodedSettings.systemInfo.organizationName == "测试组织");
    assert(decodedSettings.systemInfo.teamMemberCount == 32);
    assert(decodedSettings.systemInfo.storageDocumentBytes == 1024);
    assert(decodedSettings.systemInfo.storageVideoBytes == 2048);
    assert(decodedSettings.systemInfo.syncedFileCount == 18);
    assert(decodedSettings.systemInfo.lastFileSyncAtUtcMs == 1'725'000'100'000ULL);
